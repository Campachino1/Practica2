cadena = ''' NumPy is the fundamental package needed for scientific computing with Python.
...
cadena_md = cadena.split(' ')
palabras_cant = {'Palabra':'','cant':0}
palabras_repetidas = []
for palabra in cadena_md:
    if not (palabra in palabras_repetidas):
        palabras_repetidas.append(palabra)
        if (cadena_md.count(palabra)>palabras_cant['cant']):
            palabras_cant['Palabra']=palabra
            palabras_cant['cant']=cadena_md.count(palabra)
